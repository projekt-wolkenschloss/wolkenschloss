...
After installation is complete, further config changes that do not change the partitions can be build with

```bash
NIX_SSHOPTS="-p 22220 -i ~/.ssh/id_ed25519_wolkenschloss_test_vms" sudo nixos-rebuild build --flake .#wlknslos-single-storage-device --use-remote-sudo --target-host nixos@localhost
```
